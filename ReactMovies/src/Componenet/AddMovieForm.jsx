...
const AddMovieForm = () => {
    const [movie, setMovie] = useState({ title: '', imageUrl: '', description: '' });
...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.status === 200) {
            alert('Movie added')
            setMovie({ title: '', imageUrl: '', description: '' })
        }
    } catch (error) {
        ...
        <form className="w-1/2 mx-auto my-12 p-6 border rounded-lg shadow-lg" onSubmit={handleSubmit}>
            <div className="mb-4">
                ...
                <input type="text" name="title" value={movie.title} onChange={handleChange}
                    placeholder="Enter movie title" className="w-full p-2 border rounded-md" required />
            </div>
            <div className="mb-4">
                <label className="block mb-1 font-semibold text-white">Image URL:</label>
                <input type="text" name="imageUrl" value={movie.imageUrl} onChange={handleChange}
                    placeholder="Enter image URL" className="w-full p-2 border rounded-md" required />
            </div>
            <div className="mb-4">
                ...
                <textarea name="description" value={movie.description} onChange={handleChange}
                    placeholder="Enter movie description" className="w-full p-2 border rounded-md" required />
            </div>
